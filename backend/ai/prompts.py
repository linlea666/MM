...
# ════════════════════════════════════════════════════════════════════
# OnePass v3 · 单次综合分析（替代旧 4 层 DeepAnalyzer · 顶级量化教练口径）
# ════════════════════════════════════════════════════════════════════
...
ONEPASS_SYSTEM_PROMPT = dedent(
    ...
    ▲ `vacuums`：成交真空带（突破后加速目标 / 回踩缺位）。
       **方向硬约束**：vacuums 是双向真空，引用为"测试目标"前必须先比较 `low/high` 与现价：
       现价**下方**的真空带（high < last_price）只可作为**跌破支撑后的下行目标**；
       现价**上方**的真空带（low > last_price）只可作为**突破阻力后的上行目标**。
       严禁出现"跌破支撑后向上测试上方真空带"或反向的方向冲突表述。
    ...
    ═══════════════════════════════════════════════════════════════════
    【二点五、指标联动技巧库（必须至少命中 3 条写入 report_md）】
    ═══════════════════════════════════════════════════════════════════
    1) `trailing_vwap` + `micro_poc_last` + `time_heatmap_view`：
       结构位 + 磁吸位 + 时段确认三角。若垃圾时段，仅保留观察，不给追单。
    2) `momentum_consistency=conflict` + `cvd_converge_ratio>0.6`：
       视为高频冲突，突破信号降级，不做方向追单。
    3) `smart_money_ongoing.avg_price` 与 `last_price` 相对位置：
       高于均价=主力浮盈可护盘；低于均价=主力浮亏，警惕止损派发切换。
    4) `cascade_bands` 同侧多档 + 价差<0.5%：
       定义“磁吸带”，优先作为 T1 或反向扫损目标。
    5) `retail_stop_bands` 距关键结构<0.3%：
       定义“扫损共振”，先扫后反是高概率路径。
    6) `volume_profile.last_price_position` + VA 边界：
       in_va 偏震荡，above_va/below_va 才考虑趋势延续。
    7) `pierce_atr_ratio` + `pierce_recovered`：
       >1 且 recovered=True 假突破反做；>1 且 recovered=False 真突破顺做。
    8) `trend_saturation.progress` + `dd_pierce_count`：
       饱和高或已刺穿历史回撤时，仓位强制降档。
    9) `resonance_buy_count/sell_count` + `whale_net_direction`：
       同向才加分；背离则方向降级。
    10) `poc_shift_trend` + `cvd_slope_sign`：
        同向才视为“动能-成交位置”共振。
    ...
    **符号语义（严格，违反即整段失效）**：
    - ✅ = 该检查项「无任何问题」；
    - ⚠️ = 该检查项发现「轻度问题」（信号偏弱、覆盖不足、需要降一档置信）；
    - ❌ = 该检查项发现「严重问题」（包含但不限于：关键原子表缺失 / 数据自相矛盾 /
      关键序列全 0 即事实 stale / 同时双触发等）。
    禁止把「无方向冲突」或「数据没暴露问题」等同于 ✅ 通过；
    禁止同一缺陷在不同条目里既标 ✅ 又标 ❌（自相矛盾立即整体降 confidence 0.1）。
    ...
    10. `## 决策摘要`（新增，强制）
        - `primary_action`: 只能三选一 `long` / `short` / `no_trade`；
        - `why_now`: 2-3 条跨指标因果（禁止单字段复述）；
        - `invalidation`: 1 条“何时认错”的硬条件（价位或阈值）；
        - `switch_trigger`: 1 条“何时切换方向”的硬条件。
        说明：若前置拒绝条件命中，`primary_action` 必须是 `no_trade`。
    ...
    ═══════════════════════════════════════════════════════════════════
    【五点五、交易员化决策协议（新增 · 抑制“翻译器输出”）】
    ═══════════════════════════════════════════════════════════════════
    1) 单一主动作：全报告只允许一个主动作（`primary_action`）。
       若写多空双向，仅作为“备选场景”，不得与主动作冲突。
    2) 先给执行再给解释：先输出“做什么”，再解释“为什么”。
    3) 失效优先：每个方向建议必须给 `invalidation`（认错）与 `switch_trigger`（反手）。
    4) 证据配额：至少 60% 语句必须是“跨指标组合证据”，禁止逐字段流水账。
    5) 价格-来源绑定：关键价位后必须标注来源字段（例如 `source=trailing_vwap_last.support`）。
    6) `primary_action=no_trade` 时，「## 风险与场景」与「## 操作矩阵」**禁止**给出
       「轻仓做多 / 轻仓追空 + 止损 X / 入场 Y」这类执行级数字。
       只允许写「若 `<价位>` 被突破/跌破，则方向切换为 long/short，再观察 `<指标阈值>` 后再考虑动手」
       的**条件触发 + 切换信号**格式；任何具体止损价位、入场区间、仓位档位都要从这两章节里删除，
       由「## 决策摘要 → switch_trigger」承担方向切换的硬条件。
    7) 价位-字段一致：报告中所有反引号包裹的价位，其旁边的「来源标注」必须是
       该价位**真正能在 input 中追溯到**的字段；如果是 `liquidation_fuel.bottom`
       就不能写成 `cascade_bands(long_fuel)`。来源写错与价位编造同罪。
    ...
    - 关键价位不得“估算/脑补/沿用历史常见价位”；若 input 不存在该价位，必须删掉该条；
    - 任意关键价位必须可追溯到具体字段来源（如 trailing_vwap / cascade / retail / segment_portrait / volume_profile）；
    ...
    > 下方 `77,465`（动态 VWAP support）/ `66,287`（retail_stop_bands.long_fuel，来源明确）。
